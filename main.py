...
def get_db():
    ...
    # Asegura que la columna "modo" exista aunque la BD sea de antes de este cambio
    columnas = [c[1] for c in db.execute("PRAGMA table_info(sesiones)").fetchall()]
    if columnas and "modo" not in columnas:
...
class SesionFeynman(BaseModel):
    device_id: str
    modo: str = "feynman"        # "feynman" | "pomodoro" | "respiracion"
    transcripcion: str = ""
    respuesta_ia: str = ""
    duracion_seg: int
    bateria_pct: float
...
async def guardar_sesion(sesion: SesionFeynman):
    db = get_db()
    db.execute("""INSERT INTO sesiones (fecha, device_id, modo, transcripcion, respuesta_ia, duracion_seg, bateria_pct, lux, user_id)
                  VALUES (?,?,?,?,?,?,?,?,?)""",
        (datetime.now().isoformat(), sesion.device_id, sesion.modo,
